...
const InvoiceList = ({ navigation }) => {
    ...
    const fetchData = async () => {
        setLoadData(true)
        try {
            const apiUrl = 'https://office3i.com/api/public/api/proforma_invoice_list';
            const response = await axios.post(apiUrl, {
                email_id: "sathiya.epkgroup@gmail.com"
            },
                {
                    headers: {
    ...
    const exportToExcel = async () => {
        const tableHead = ['S.No', 'Invoice Number', 'Name', 'E-mail', 'Plan Name', 'Add on Employee', 'Total Amount', 'Payment Method', 'Payment Status', 'Plan Type', 'Start Date', 'End Date'];
        ...
        try {
            ...
            const fileUri = RNFS.CachesDirectoryPath + '/Trial_Pack_List.xlsx';
                        ...
                        {loadData ? (
                            ...
                            <View>
                                ...
                                <View style={[styles.row, styles.listHeader]}>
                                    ...
                                    <Text style={[styles.header, styles.cell, styles.WeekOff]}>Invoice Number</Text>
                                    <Text style={[styles.header, styles.cell, styles.EndDate]}>Name</Text>
                                    <Text style={[styles.header, styles.cell, styles.EndDate]}>E-mail</Text>
                                ...
                                {paginatedData.length === 0 ? (
                                    ...
                                    paginatedData.map((item, index) => (
                                        <View key={index} style={[styles.row, styles.listBody]}>
                                            ...
                                            <Text style={[styles.cell, styles.WeekOff]}>{item.transaction_id}</Text>
                                            <Text style={[styles.cell, styles.EndDate]}>{item.cus_name}</Text>
                                            <Text style={[styles.cell, styles.EndDate]}>{item.cus_email}</Text>
                                            ...
                                            <View style={styles.listcontentButtonview}>
                                                <TouchableOpacity style={styles.listcontentviewbutton}
                                                    onPress={() => navigation.navigate('View Proforma Invoice', { Id: item })}
                                                >
                                                    <ViewIcon width={14} height={14} color={"#000"} />
                                                </TouchableOpacity>
                                                <TouchableOpacity style={styles.listcontenteditbutton}
                                                    onPress={() => navigation.navigate('Edit Proforma Invoice List', { Id: item })}
                                                >
